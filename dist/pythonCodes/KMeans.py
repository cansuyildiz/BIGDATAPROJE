...
def evaluate(train,predictors,k):  
    kmeansEstimator = H2OKMeansEstimator(k=k, init="Random", seed=2, standardize=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='K-Means on Sparkling Water')
    parser.add_argument('--dataset', type=str, default="winequality-red2.csv")
    parser.add_argument('--k', type=int, default=3)

    args = parser.parse_args()

    data_filepath="hdfs://localhost:9000/user/cansu/" + args.dataset   #reaction_network.txt"
    k = args.k
    ...
    conf = SparkConf().setAppName("KMeans").setMaster("local[*]").set("spark.ui.port", "7077").set("spark.executor.memory", "5g").set("spark.driver.memory", "5g")
    ...
    for i in range(2):
        t1=time.time()
        estimator = evaluate(train,predictors,k)
        t2=time.time()
    ...
    performance = getPrediction(estimator,test)
